...
// -----------------------------------------------------------------------
// Homepage: inject WebSite + Organization JSON-LD into dist/index.html.
// This tells Google that bpad.pro is a known legitimate entity with a login
// form — reducing phishing false positives from Safe Browsing heuristics.
// The visible product copy lives in Landing.tsx (rendered by React).
// -----------------------------------------------------------------------
...
const updatedIndex = distHtml.replace('</head>', `  ${websiteJsonLd}\n  </head>`)
fs.writeFileSync(path.join(distDir, 'index.html'), updatedIndex)
console.log(`updated dist/index.html with WebSite/Organization JSON-LD`)
